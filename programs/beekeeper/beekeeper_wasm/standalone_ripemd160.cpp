...
std::array<uint8_t, 20> standalone_ripemd160(const uint8_t* data, size_t len)
{
  ...
  // Padding
  size_t bufsize = static_cast<size_t>(bytes % 64);
  buf[bufsize++] = 0x80;
  ...
  WriteLE32(result.data() + 0,  s[0]);
  WriteLE32(result.data() + 4,  s[1]);
  WriteLE32(result.data() + 8,  s[2]);
  WriteLE32(result.data() + 12, s[3]);
  WriteLE32(result.data() + 16, s[4]);
  return result;
}
